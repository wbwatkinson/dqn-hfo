...
DEFINE_int32(pad, 0, "Number of features to pad state inputs");
DEFINE_bool(pad_zero, true, "Pad state features with 0");
DEFINE_int32(gpu_device, 0, "Device to run algorithm");

DEFINE_int32(port, 0, "Port on which to run rcssserver. Default is to select based on a random value generated by seeding the save path");
...
std::vector<float> padded_state(HFOEnvironment& hfo, dqn::DQN& dqn) {
  std::default_random_engine generator;
  const std::vector<float>& current_state = hfo.getState();

  std::vector<float> current_state_copy;
  std::copy(current_state.begin(), current_state.end(), back_inserter(current_state_copy));

  if (FLAGS_pad > 0) {
    for (int i = 0; i<FLAGS_pad; ++i) {
      if (FLAGS_pad_zero) {
        current_state_copy.push_back(0);
      } else {
        current_state_copy.push_back(std::uniform_real_distribution<float>(-1.0,1.0)(generator));
      }
    }
  }
...
std::tuple<double, int, status_t, double> PlayOneEpisode(HFOEnvironment& hfo,
  ...
  while (!game.episode_over) {
    const std::vector<float>& current_state = padded_state(hfo, dqn); //hfo.getState();
    ...
    if (past_states.size() < dqn::kStateInputCount) {
      ...
      if (update) {
        const std::vector<float>& next_state = padded_state(hfo, dqn); //hfo.getState();
...
double Evaluate(HFOEnvironment& hfo, dqn::DQN& dqn, int tid) {
  ...
  LOG(INFO) << "[Agent" << tid << "] Evaluation: "
            ...
            << ", critic_loss = " << dqn.critic_loss()
            << ", actor_loss = " << dqn.actor_loss();
  return goal_percent;
}
...
void KeepPlayingGames(int tid, std::string save_prefix, int port) {
  ...
  if (FLAGS_gpu) {
    ...
    //exit(0);
    caffe::Caffe::SetDevice(FLAGS_gpu_device);
  } else {
  ...
  int num_features = NumStateFeatures(num_players) + FLAGS_pad;
  LOG(INFO) << num_features << " state features";
