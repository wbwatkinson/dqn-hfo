...
do
	...
	SAVE=~/projects/dqn-hfo/state/test/$JOB
	PORT=$(($port_base + ($num_agents + 1) * $count))
	PID="~/projects/dqn-hfo/bin/dqn --save $SAVE/ddpg --nogpu --noremove_old_snapshots --offense_agents 1 --defense_npcs 1 --hfo-logging --log-dir log/test pwd--resequence-features --port $PORT"
	launch $JOB $SAVE "$PID"
	count=$(($count+1))
done
